...
test('claude-code renders POSIX env block', () => {
  const block = __test__.envBlock({ baseUrl: 'https://hypershub.com/v1', key: 'hy-test', model: 'claude-sonnet-4-6' }, 'posix')
  assert.match(block, /export ANTHROPIC_BASE_URL="https:\/\/hypershub.com"/)
  assert.match(block, /export ANTHROPIC_AUTH_TOKEN="\$HYPERSHUB_API_KEY"/)
})

test('claude-code renders fish env block', () => {
  const block = __test__.envBlock({ baseUrl: 'https://hypershub.com', key: 'hy-test', model: 'claude-sonnet-4-6' }, 'fish')
  assert.match(block, /set -gx ANTHROPIC_BASE_URL "https:\/\/hypershub.com"/)
})

test('claude-code renders PowerShell user env block', () => {
  const block = __test__.envBlock({ baseUrl: 'https://hypershub.com', key: 'hy-test', model: 'claude-sonnet-4-6' }, 'powershell')
  assert.match(block, /SetEnvironmentVariable\("ANTHROPIC_BASE_URL", "https:\/\/hypershub.com", "User"\)/)
...
test('claude-code syncs settings.json for cc-switch compatibility', () => {
  ...
  try {
    ...
    __test__.syncClaudeSettings({ baseUrl: 'https://hypershub.com/v1', key: 'hy-test', model: 'claude-deepseek-v4-pro' })
    const cfg = JSON.parse(fs.readFileSync(settings, 'utf8'))
    assert.equal(cfg.env.ANTHROPIC_BASE_URL, 'https://hypershub.com')
